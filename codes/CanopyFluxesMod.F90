...
! !USES:
  ...
  !Ashehad Ali added clm_time_manager above
...
!KO   tpuc   = fth25 (tpuhd, tpuse)
   ...
   do f = 1, fn
      ...
      !call get_curr_date(kyr, kmo, kda, mcsec)  ! Ashehad Ali added below to consider for dynamic SLA
   
            !if (kmo < 4) then
            !slatop(ivt(p)) = 0.82_r8 * 0.024_r8
	    !else if (kmo >= 4 .and. kmo <= 5)  then
            !slatop(ivt(p)) = 0.82_r8 * 0.024_r8
            !else if (kmo >= 6 .and. kmo <= 7)  then
            !slatop(ivt(p)) = 1.0_r8 * 0.024_r8  
