...
!-----------------------------------------------------------------------
   ...
   call CNStressDecidPhenologyRubNew(num_soilp, filter_soilp)
...
!BOP
!
! !IROUTINE: CNStressDecidPhenologyRubNew
!
! !INTERFACE:
subroutine CNStressDecidPhenologyRubNew (num_soilp, filter_soilp)
!
! !DESCRIPTION:
! This routine handles phenology for vegetation types, such as grasses and
! tropical drought deciduous trees, that respond to cold and drought stress
...
! !USES:
   ...
   use clm_varcon      , only: secspday
   use shr_const_mod   , only: SHR_CONST_TKFRZ, SHR_CONST_PI
   use pftvarcon       , only: rain_min, crit_dry_dur
!
! !ARGUMENTS:
   integer, intent(in) :: num_soilp       ! number of soil pfts in filter
   integer, intent(in) :: filter_soilp(:) ! filter for soil pfts
...
! !OTHER LOCAL VARIABLES:
   ...
   real(r8):: lat             !latitude (radians)
   real(r8):: temp            !temporary variable for daylength calculation   
   real(r8):: water_avail_grow        ! water on top of soil available for growth
   integer kyr                        ! current year
   integer kmo                        ! month of year  (1, ..., 12)
   integer kda                        ! day of month   (1, ..., 31)
   integer mcsec                      ! seconds of day (0, ..., seconds/day)
   ...
   do fp = 1,num_soilp
      ...
      if (stress_decid(ivt(p)) == 1._r8) then
         ...
         if (dormant_flag(p) == 1._r8) then
            ...
            ! if soils are wet, accumulate soil water index for onset trigger
            !if (psi >= soilpsi_on) onset_swi(p) = onset_swi(p) + fracday
	    !if (water_avail_grow >= rain_min(ivt(p)) / (3600._r8 * 24._r8 * 30._r8)) onset_swi(p) = onset_swi(p) + fracday   ! Ashehad Ali rain_min is 50mm/month
	     if (psi >= -2.34_r8) onset_swi(p) = onset_swi(p) + fracday ! -0.07, -2.34 before
         ...
         ! test for switching from growth period to offset period
            ...
            !if (psi <= soilpsi_off) then          
	    !if (water_avail_grow <= rain_min(ivt(p)) / (3600._r8 * 24._r8 * 30._r8)) then   !Ashehad Ali
	     if (psi <= -2.34_r8) then
               offset_swi(p) = offset_swi(p) + fracday
	 ...
	 call get_curr_date(kyr, kmo, kda, mcsec)  ! Ashehad Ali added this
   
            if (kmo < 4) then
            leaf_long(ivt(p)) = 2.0_r8
	    else if (kmo >= 4 .and. kmo <= 5)  then
            leaf_long(ivt(p)) = 1.55_r8 
            else if (kmo >= 6 .and. kmo <= 7)  then
            leaf_long(ivt(p)) = 0.55_r8  
	    else if (kmo >= 8 .and. kmo <= 9)  then
            leaf_long(ivt(p)) = 0.23_r8                         
            else
            leaf_long(ivt(p)) = 4.1_r8 
            end if
...
end subroutine CNStressDecidPhenologyRubNew
!-----------------------------------------------------------------------
