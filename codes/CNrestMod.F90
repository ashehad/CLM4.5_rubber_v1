...
!-----------------------------------------------------------------------
   ...
   ! fert
    ...
    ! tappingc
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='tappingc', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='tappingc', data=pcs%tappingc, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if
    ...
    ! tappingn
    if (flag == 'define') then
       call ncd_defvar(ncid=ncid, varname='tappingn', xtype=ncd_double,  &
            dim1name='pft',long_name='',units='')
    else if (flag == 'read' .or. flag == 'write') then
       call ncd_io(varname='tappingn', data=pns%tappingn, &
            dim1name=namep, ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) then
  	  if (is_restart()) call endrun
       end if
