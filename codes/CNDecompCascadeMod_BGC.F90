      ...
      do j = 1, nlevdecomp
         do fc = 1,num_soilc
            c = filter_soilc(fc)
               ...
               if (t_soisno(c,j) .ge. SHR_CONST_TKFRZ) then
                  t_scalar(c,j)= (3.0**((t_soisno(c,j)-(SHR_CONST_TKFRZ+25._r8))/10._r8))        ! Ashehad Ali modifed 1.5 to 3.0
               else
                  t_scalar(c,j)= (3.0**(-25._r8/10._r8))*(froz_q10**((t_soisno(c,j)-SHR_CONST_TKFRZ)/10._r8))   ! Ashehad Ali modifed 1.5 to 3.0
               endif
         end do
      end do
